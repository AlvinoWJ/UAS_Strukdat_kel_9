...
int isNamaKelasUnik(DaftarKelas *daftar, char *namaKelas) {
    ...
    while (current != NULL) {
        if (strcmp(current->namaKelas, namaKelas) == 0) {
            return 0; // Nama kelas tidak unik
        }
        current = current->next;
    }
    return 1; // Nama kelas unik
}
...
// menu utama
int main() {
    DaftarKelas daftarKelas = {NULL, NULL, 0};
    Que que;
    int pilihan;
    ...
    do {
        printf("\n=== Menu Utama ===\n");
        printf("1. Menu Kelas\n");
        printf("2. Menu Aktivitas\n");
        printf("3. Menu Menu Belajar\n");
        printf("99. Keluar\n");
